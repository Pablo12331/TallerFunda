...
static void generate_statement_code(Node* node, std::stringstream& out, int indent_level) {
    ...
    switch (node->type) { //Toma el tipo del puntero
        ...
        case NODE_VAR_DECLARATION: { //Si es un NODE_VAR_DECLARATION
            out << indent(indent_level); //genera la respectiva identacion si es que lo necesita
            VarDeclarationNode* decl_node = (VarDeclarationNode*)node; //Guarda en un puntero auxiliar el puntero otorgado
